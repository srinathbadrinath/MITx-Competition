...
HeadlineWordsTrain$Hour = NewsTrain$PubDate$hour



# check if headline is a question or has quotes

questionVector = c("can","how","why","are","is","should","what","when","will","do","who","where","would","don't")
...
HeadlineWordsTest$Hour = NewsTest$PubDate$hour
...
HeadlineWordsLog = glm(Popular ~ NewsDesk + SectionName + SubsectionName + WordCount + Weekday + Hour + HLQuestion, data=HeadlineWordsTrain, family=binomial)

PredTest = predict(HeadlineWordsLog, newdata=HeadlineWordsTest, type="response")

MySubmission = data.frame(UniqueID = NewsTest$UniqueID, Probability1 = PredTest)

write.csv(MySubmission, "SubmissionWordGLM8.csv", row.names=FALSE)
