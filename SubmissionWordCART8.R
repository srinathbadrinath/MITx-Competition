...
HeadlineWordsLog = glm(Popular ~ NewsDesk + SectionName + SubsectionName + WordCount + Weekday + HLQuestion, data=HeadlineWordsTrain, family=binomial)

PredTest = predict(HeadlineWordsLog, newdata=HeadlineWordsTest, type="response")

MySubmission = data.frame(UniqueID = NewsTest$UniqueID, Probability1 = PredTest)

write.csv(MySubmission, "SubmissionWordGLM7.csv", row.names=FALSE)
...
tr = train(Popular ~ NewsDesk + SectionName + SubsectionName + WordCount + Weekday + HLQuestion, data=HeadlineWordsTrain, method = "rpart", trControl = tr.control, tuneGrid = cp.grid)
...
write.csv(MySubmission, "SubmissionWordCART8.csv", row.names=FALSE)
